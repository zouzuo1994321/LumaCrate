...
[![Version](https://img.shields.io/badge/version-v1.30.0-blue.svg)](https://github.com/zouzuo1994321/LumaCrate/releases)
...
[![Build](https://img.shields.io/badge/build-2609210040-success.svg)](https://github.com/zouzuo1994321/LumaCrate/releases)
...
- **Actor cards show the work count** (v1.30.0) — the card's 5th fact is now **works** instead of bust size. Third row "三围" already reads `bust85·waist58·hip83`, so a separate bust column was pure duplication, whereas "how many titles" is the first thing you look at. Still **3 rows**, still `236×160px`.
- **A–Z letter index in the actor & director libraries** (v1.30.0) — a vertical `A–Z + #` rail sits **next to the scrollbar**; clicking a letter scrolls straight to the first person under it. Letters with nobody are dimmed and inert. The sort key prefers **`romaji`** (4,766 of 5,909 real actors have one), falls back to the name's first character, and buckets non-Latin characters into **#**. If the current sort isn't "letter ↑", clicking a letter **switches the sort first and rebuilds** — jump positions are computed for that ordering.
...
- Go to **[GitHub Releases](https://github.com/zouzuo1994321/LumaCrate/releases)** and download the latest `流明盒-v1.30.0-<build>.exe`;
