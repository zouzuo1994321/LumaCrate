...
from letter_index import LetterIndexBar          # v1.30.0：演员库 / 导演库右侧 A-Z 索引
...
class LazyGrid(QWidget):
    ...
    def _jump_step(self):
        ...
        if self._loaded > tgt:
            ...
            return
        self._busy = True
        want = min(max(self.batch, 240), max(0, self._total - self._loaded))
        try:
...
class MainWindow(QMainWindow):
    ...
    def _view_directors(self):
        ...
        return self._people_rail(page, "Director", filt, sort, asc, grid)
    ...
    def _start_smart(self, page_no):
        ...
        self._smart_worker = SmartWorker(
            page=page_no, limit=int(rec.get("count", 24)), algo=self._smart_algo, parent=self)
    ...
    def _view_actors(self):
        ...
        return self._people_rail(page, "Actor", filt, sort, asc, grid)

    # ---------- A-Z 字母索引导航（v1.30.0 反馈 4） ----------
    def _people_rail(self, page_scroll, role_type, filt, sort, asc, grid):
        """给演员库 / 导演库页面套一层右侧 A-Z 索引条，返回可直接当页面的容器。

        为什么不塞进 `_page`：索引条要**贴在滚动条旁边**，跟着内容一起滚就失去
        「随时跳」的意义了 → 它必须在 QScrollArea **外面**。于是拿 `_page` 返回的
        滚动区当左列、索引条当右列重新装一个 QWidget。

        跳转下标是按「首字母升序」这套顺序算出来的（`db.people_letter_index`），
        所以：当前不是该排序时，先把偏好切成 首字母↑ 再整页重建，重建完自动落到
        刚才点的那个字母（见各 view 里的 `_pending_letter`）；否则直接跳。
        """
        try:
            index = db.people_letter_index(role_type=role_type, asc=True, **filt)
        except Exception as e:
            # 索引条只是加速锦上添花，出问题绝不能把页面搞没 → 退回不带索引的原页面
            applog.log(f"字母索引生成失败：{e}", "error")
            return page_scroll
        if not index:
            return page_scroll

        rail = LetterIndexBar(index)
        box = QWidget()
        h = QHBoxLayout(box)
        h.setContentsMargins(0, 0, 0, 0)
        h.setSpacing(0)
        h.addWidget(page_scroll, 1)
        h.addWidget(rail)
        box.letter_bar = rail
        box.lazy_grid = grid            # 供离屏冒烟直接取到网格，不用翻 layout

        def go(letter):
            pos = rail.position_of(letter)
            if pos is None:
                return
            if sort != "letter" or not asc:
                cfg.get_settings().set_actor_prefs("letter", True, dict(filt))
                self._pending_letter = (role_type, letter)
                self._replace_current(self._people_builder or self._view_actors)
                return
            rail.set_active(letter)
            grid.jump_to(pos, lambda card: self._scroll_to_card(page_scroll, card))

        rail.picked.connect(go)

        # 排序刚被切成「首字母↑」重建出来的这一页：把用户点的那个字母接住，自动跳过去
        pend = getattr(self, "_pending_letter", None)
        if pend and pend[0] == role_type:
            self._pending_letter = None
            QTimer.singleShot(0, lambda lt=pend[1]: rail.click_letter(lt))
        return box

    def _scroll_to_card(self, page_scroll, card):
        """把目标卡片滚进视野（上下各留一点余量，别贴边）。"""
        if card is None or not _qt_alive(card, page_scroll):
            return
        try:
            page_scroll.ensureWidgetVisible(card, 80, 140)
        except RuntimeError:
            pass
