...
class SettingsDialog(QDialog):
    ...
    def _build(self):
        ...
        # v1.30.0（反馈 2 / 3）：「手动修改」插到「个性化设置」与「画像概览」之间；
        #                       「图像检测」插到「重复检测」与「数据与日志」之间。
        self.ORDER = ["个性化设置", "手动修改", "画像概览", "演员刮削", "智能推荐",
                      "标签优化", "服务管理", "重复检测", "图像检测", "数据与日志"]
        ...
        for pg in (self._pg_personal, self._pg_manual, self._pg_insight, self._pg_scraper,
                   ...
                   self._pg_imagedetect, self._pg_data):
