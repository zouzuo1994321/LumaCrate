...
一轮好几分钟）；本脚本只盯 v1.30.0 的四条反馈，几十秒跑完：
  ...
  1. 窗口标题带 v1.30.0（Build 2609240041）
...
def diff_ratio(a, b, step=6, x0=200, y0=70):
    """两张抓图在内容区里的差异像素占比，用来判断「画面还在变 / 已经稳了」。"""
    if a is None or b is None:
        return 1.0
    ia, ib = a.toImage(), b.toImage()
    h = min(ia.height(), ib.height()) - 10
    w = min(ia.width(), ib.width()) - 40
    tot = n = 0
    for y in range(y0, max(y0 + 1, h), step):
        for x in range(x0, max(x0 + 1, w), step):
            pa, pb = ia.pixelColor(x, y), ib.pixelColor(x, y)
            tot += 1
            if abs(pa.red() - pb.red()) + abs(pa.green() - pb.green()) \
                    + abs(pa.blue() - pb.blue()) > 12:
                n += 1
    return n / max(1, tot)
...
def main():
    ...
    ok_title = "v1.30.0" in title and "2609240041" in title
    print(" [%s] 标题带 v1.30.0 (Build 2609240041)" % ("PASS" if ok_title else "FAIL"), flush=True)
    ...
    # 跳转要按批补载到目标行（离屏实测 180→900 张约 8.5s，真机读 NAS 图更慢），
    # 所以这里分时段连拍，看画面是「慢慢加载出来」还是「压根没画」。
    for tag, wait in (("a", 4), ("b", 10), ("c", 20), ("d", 30)):
        time.sleep(wait)
        shot(app, win, "92%s_live_actors_jump_C.png" % tag, settle=4)

    # ---- 跳到最远的 'Z'：量真机耗时（离屏 25.9s）----
    # 第 26 行 = 'Z'（A=0 … Z=25）
    rail_y_z = int(top + row * (25 + 0.5))
    print(" [点击] 索引条 'Z' @(%d,%d)" % (rail_x, rail_y_z), flush=True)
    prev = grab(app, win, settle=2)
    t_click = time.time()
    click(win, rail_x, rail_y_z)
    stable, elapsed, moved = 0, None, False
    while time.time() - t_click < 150:
        time.sleep(1.0)
        app.processEvents()
        cur = grab(app, win, settle=0)
        if cur is None or cur.width() <= 1:
            continue
        r = diff_ratio(prev, cur)
        if r > 0.01:
            moved = True
            stable = 0
        else:
            ...
            if moved and stable >= 2:
                ...
                break
        prev = cur
    print(" [跳转] 'Z'：%s（画面已变化=%s）"
          % ("%.1fs 后画面稳定" % elapsed if elapsed else "150s 内未稳定", moved), flush=True)
    shot(app, win, "97_live_actors_jump_Z.png", settle=4)
