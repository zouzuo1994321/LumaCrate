...
def ai_review(result, model=None, max_clusters=25, max_suspects=15,
    ...
    out = {"ai": ok, "model": "", "note": note, "done": 0, "failed": 0}
    if not ok:
        return out
    try:
    ...
    jobs = [("cluster", c) for c in result.get("clusters", [])[:max_clusters]]
    jobs += [("suspect", s) for s in result.get("suspects", [])[:max_suspects]]
