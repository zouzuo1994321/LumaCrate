...
class ImageScanWorker(QThread):
    ...
    def __init__(self, library, slots, parent=None):
...
class ImageDetectPage(QWidget):
    ...
    def _run(self):
        ...
        self._worker = ImageScanWorker(self.current_library(), slots, parent=self)
    ...
    def _on_done(self, rep):
        ...
        self.status_lbl.setText(
            ...
            f"· 耗时 {s['elapsed']} 秒 · 当前列出 {shown} 条{extra}")
    ...
    def _fill_tree(self, rows):
        ...
        try:
            for i, p in enumerate(rows[:self.MAX_ROWS]):
                ...
                it.setToolTip(3, p.get("detail") or "")
    ...
    def _mark_fixed(self, row):
        ...
        it = items[0]
        it.setText(2, "已修复")
        it.setText(3, os.path.basename(row.get("path") or ""))
        f = it.font(0)
        f.setStrikeOut(True)
        for c in range(4):
            it.setFont(c, f)
