...
# ============================================================ A. 版本号
section("A. 版本号（v1.30.0 / Build 2609240041）")
check("A1 外部版本 v1.30.0", ver.VERSION == "v1.30.0", ver.VERSION)
check("A2 内部构建号 2609240041（顺延）", ver.BUILD == "2609240041", ver.BUILD)
check("A3 完整标识", ver.FULL_VERSION == "v1.30.0 (Build 2609240041)", ver.FULL_VERSION)
...
# ---------------- I. 头像按需加载（v1.30.0 性能修复） ----------------
# 真机 5909 位演员，A-Z 跳到 'Z' 要补建 4800 张卡；原先在 _build 里同步读 NAS 图
# （冷读 ~5.7ms/张）整趟 59s。改成「画到才读」后整趟 ~26s，首屏/滚动也一起变快。
# 这里锁住两件事：① 没画过的卡**不读盘** ② 画过的卡**必须真有图**（不能永远占位）。
print("\n--- I. 头像按需加载 ---")
_ava_path = os.path.join(mdir, "_avatar_test.jpg")
_jpg(_ava_path, 320, 480)
_card = mw.ActorCard(
    {"name": "测试演员", "thumb": _ava_path, "meta": '{"身高": "160cm"}',
     "status": "现役", "works": 3},
    lambda _p: None, lambda _p: None, lambda _p: None, lambda _c: None, None)
check("I1 未绘制的卡头像处于 pending（不读盘）",
      _card._avatar_pending is True and bool(_card._avatar_path))
_pm0 = _card._avatar.pixmap()
_card.show()
pump(4)
_pm1 = _card._avatar.pixmap()
check("I2 绘制后头像真的读进来了（pending 落闸）", _card._avatar_pending is False)
check("I3 绘制后头像 pixmap 非空且尺寸正确",
      (not _pm1.isNull()) and _pm1.width() == mw._ACTOR_AVATAR > 0,
      (_pm1.width(), _pm1.height()))
_card2 = mw.ActorCard(
    {"name": "无图演员", "thumb": "", "meta": "{}", "works": 0},
    lambda _p: None, lambda _p: None, lambda _p: None, lambda _c: None, None)
_card2.show()
pump(4)
check("I4 本来就没图的演员：无 pending、不报错、占位圆仍在",
      _card2._avatar_pending is False and not _card2._avatar.pixmap().isNull())
_card2.close()
_card.close()
