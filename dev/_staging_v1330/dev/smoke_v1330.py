...
跑法（shim 下必须走 runpy）：
    python -u -c "import runpy; runpy.run_path(r'<本项目>/dev/smoke_v1330.py', run_name='__main__')"
...
TMP = os.path.join(tempfile.gettempdir(), "lmc_smoke_v1330")
...
# ============================================================ A 版本号
section("A 版本号")
check("A1 VERSION == v1.33.0", ver.VERSION == "v1.33.0", ver.VERSION)
check("A2 BUILD == 2609240044", ver.BUILD == "2609240044", ver.BUILD)
check("A3 FULL_VERSION 同步", ver.FULL_VERSION == "v1.33.0 (Build 2609240044)",
      ver.FULL_VERSION)
_vs = src("version.py")
check("A4 version.py 顶部注释含本版四条反馈摘要",
...
check("E1b 渲染后 _dd_report 已就位（导出 CSV/JSON 可用）",
...
check("F3 _update_head 未接管工具行时**主动 hide**（不再只 return）",
      "if not self._toolbar_ready:\n            self.head.setVisible(False)" in _si
      or "self.head.setVisible(False)\n            return" in _si)
...
try:
    ...
    for i in range(20):
        _c.execute(
            "INSERT INTO media(title,sort_title,year,library,file_path,nfo_path,"
            "quality,file_size,runtime,poster,fanart,thumb,parent_id,added_time)"
            " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            ("标题%02d" % i, "标题%02d" % i, 2020, "L1",
...
for _tag, _gs in _lg_found.items():
    for _i, _g in enumerate(_gs):
        ...
        _head_vis = _g.head.isVisibleTo(_g)
...
check("H12 正文版本行仍是新版", "v1.33.0" in _body and "2609240044" in _body,
      _body.splitlines()[0][:60] if _body else "")
check("H13 自绘图标函数存在四个分支",
      all(('kind == "%s"' % k) in _si for k in ("github", "bilibili", "weibo", "mail")))
...
check("I10 README 徽章已更新到 v1.33.0", "v1.33.0" in _rmd)
check("I11 README 迭代记录有 v1.33.0 条目", "v1.33.0" in _rmd)
_ren = io.open(os.path.join(ROOT, "README_EN.md"), encoding="utf-8").read()
check("I12 README_EN 也提到 v1.33.0", "v1.33.0" in _ren or "1.33.0" in _ren)
