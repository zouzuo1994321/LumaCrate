...
class AboutDialog(QDialog):
    ...
    * **免责与隐私如实写明** —— 本软件不联网刮削、不上传任何东西、AI 完全本地；
    ...
    # ---------------------------------------------------------------- 联系图标
    @staticmethod
    def _contact_icon(kind, size=22, tint="#e8e0d4"):
        """按 `version.CONTACTS` 的 key 画一个小图标（QIcon）。

        为什么自绘：① 单文件 exe 里 --add-data 图片路径容易出岔子（v1.23.0 图标链
        踩过）；② 用字符（如 "" "✉"）会因字体缺字变方块，且字形 side bearing 不对称
        永远不对齐（v1.21.0 播放键同款坑）。QPainterPath 画出来是确定的东西。
        """
        ...
        s = float(size)
        if kind == "github":
            # 猫头轮廓：一个圆头 + 两只耳 + 下方身体（简化到一眼能认出是 GitHub）
            p.setPen(Qt.NoPen); p.setBrush(col)
            p.drawEllipse(QRectF(s * 0.10, s * 0.14, s * 0.80, s * 0.72))
            ear_l = QPolygonF([QPointF(s * 0.14, s * 0.26), QPointF(s * 0.30, s * 0.08),
                               QPointF(s * 0.34, s * 0.30)])
            ear_r = QPolygonF([QPointF(s * 0.86, s * 0.26), QPointF(s * 0.70, s * 0.08),
                               QPointF(s * 0.66, s * 0.30)])
            p.drawPolygon(ear_l); p.drawPolygon(ear_r)
            p.setBrush(QColor("#1c1714"))            # 挖出两只眼（用父窗口底色填）
            p.drawEllipse(QRectF(s * 0.32, s * 0.40, s * 0.13, s * 0.16))
            p.drawEllipse(QRectF(s * 0.55, s * 0.40, s * 0.13, s * 0.16))
        elif kind == "bilibili":
            # 电视机 + 顶部两根天线（B 站标志性轮廓）
            p.setPen(Qt.NoPen); p.setBrush(col)
            p.drawRoundedRect(QRectF(s * 0.10, s * 0.30, s * 0.80, s * 0.56),
                              s * 0.14, s * 0.14)
            pen = QPen(col); pen.setWidthF(max(1.6, s * 0.09)); pen.setCapStyle(Qt.RoundCap)
            p.setPen(pen); p.setBrush(Qt.NoBrush)
            p.drawLine(QPointF(s * 0.32, s * 0.30), QPointF(s * 0.22, s * 0.10))
            p.drawLine(QPointF(s * 0.68, s * 0.30), QPointF(s * 0.78, s * 0.10))
            p.setPen(Qt.NoPen); p.setBrush(QColor("#1c1714"))
            p.drawEllipse(QRectF(s * 0.30, s * 0.48, s * 0.11, s * 0.16))
            p.drawEllipse(QRectF(s * 0.59, s * 0.48, s * 0.11, s * 0.16))
        elif kind == "weibo":
            # 一只「眼睛 + 下弧」—— 微博的标志性元素
            p.setPen(Qt.NoPen); p.setBrush(col)
            p.drawEllipse(QRectF(s * 0.08, s * 0.22, s * 0.72, s * 0.60))
            p.setBrush(QColor("#1c1714"))
            p.drawEllipse(QRectF(s * 0.24, s * 0.36, s * 0.26, s * 0.32))
            pen = QPen(col); pen.setWidthF(max(1.6, s * 0.10)); pen.setCapStyle(Qt.RoundCap)
            p.setPen(pen); p.setBrush(Qt.NoBrush)
            p.drawArc(QRectF(s * 0.52, s * 0.10, s * 0.40, s * 0.34), -60 * 16, 150 * 16)
        elif kind == "mail":
            # 信封：一个方框 + 一个 V 形折线
            p.setPen(Qt.NoPen); p.setBrush(col)
