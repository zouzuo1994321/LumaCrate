# -*- coding: utf-8 -*-
"""v1.33.2 离屏冒烟回归 —— 四条反馈逐条自证 + 关键回归 + 智能推荐 A+B

跑法（shim 下必须走 runpy）：
    python -u -c "import runpy; runpy.run_path(r'<本项目>/dev/smoke_v1332.py', run_name='__main__')"
...
覆盖：
  ...
  J 智能推荐 A+B：稀有度折减曲线 / 收藏人折入在归一化之前 / idf 保持原式 / 行为对比
"""
import io
import json
import os
...
TMP = os.path.join(tempfile.gettempdir(), "lmc_smoke_v1332")
...
# ============================================================ A 版本号
section("A 版本号")
check("A1 VERSION == v1.33.2", ver.VERSION == "v1.33.2", ver.VERSION)
check("A2 BUILD == 2609240046", ver.BUILD == "2609240046", ver.BUILD)
check("A3 FULL_VERSION 同步", ver.FULL_VERSION == "v1.33.2 (Build 2609240046)",
      ver.FULL_VERSION)
_vs = src("version.py")
...
check("I10 README 徽章已更新到 v1.33.2", "version-v1.33.2" in _rmd)
check("I11 README 迭代记录有 v1.33.2 条目", "### v1.33.2" in _rmd)
check("I11b README 迭代记录仍保留 v1.33.1 条目（历史不删）", "### v1.33.1" in _rmd)
check("I11c README 迭代记录仍保留 v1.33.0 条目（历史不删）", "### v1.33.0" in _rmd)
check("I11d README 徽章 build 同步到 2609240046", "build-2609240046" in _rmd)
_ren = io.open(os.path.join(ROOT, "README_EN.md"), encoding="utf-8").read()
check("I12 README_EN 也提到 v1.33.2", "v1.33.2" in _ren)
check("I12b README_EN 徽章 build 也同步", "build-2609240046" in _ren)
