...
def _fav_person_weight(works: int) -> float:
    ...
    折减曲线（FAV_PEOPLE_BASE = 1.2 时）：

        作品数   1 -> 0.85      10 -> 0.66      37（中位）-> 0.52
        100 -> 0.43            232 -> 0.35     357 -> 0.33

    即「1 部作品的冷门艺人」拿到约 2.6 倍于「357 部头部艺人」的权重，
...
class Recommender:
    ...
    def build_profile(self, rows=None, actors=None, directors=None, progress=None):
        ...
        #   公式 fav_w(t) = FAV_PEOPLE_BASE / (1 + log(1 + works))，再乘维度基础权重。
        #   357 部 → 约 0.30 倍；37 部（中位数）→ 约 0.47 倍；1 部 → 约 0.71 倍。
        work_counts = {}
