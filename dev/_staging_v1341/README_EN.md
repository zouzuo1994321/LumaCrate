...
[![Version](https://img.shields.io/badge/version-v1.34.1-blue.svg)](https://github.com/zouzuo1994321/LumaCrate/releases)
...
[![Build](https://img.shields.io/badge/build-2609250048-success.svg)](https://github.com/zouzuo1994321/LumaCrate/releases)
...
- Go to **[GitHub Releases](https://github.com/zouzuo1994321/LumaCrate/releases)** and download the latest `流明盒-v1.34.1-<build>.exe`;
...
- **v1.34.1** (2026-09-25) — Three UI defects reported via screenshots, all root-caused and fixed, each backed by a re-runnable regression probe (`dev/_scratch/overlap_v1341.py`). **(1) Numbers overlapping their own borders in "Auto-fill from portrait".** `style.qss` gives `QSpinBox`/`QDoubleSpinBox` a `padding: 5px 10px`, so 10px is eaten on each side of the content box; add `0.5 ~ 10.0`, one decimal and the up/down arrows and Qt reports `minimumSizeHint().width() == 131px` on this machine — but the code hard-coded `setFixedWidth(96)` / `setFixedWidth(88)`, i.e. **35–43px short of the minimum**, so Qt crushed the digits and the arrows into one cell: digits flush against the edge, right side clipped. Fixed with a single source of truth, `config.SPIN_MIN_W = 131` / `SPIN_MAX_W = 160`, applied to all three spin boxes (auto-fill "top N", auto-fill "weight", and the guide-weight box on the recommend wall) as `setMinimumWidth` + `setMaximumWidth` + `SizePolicy(Expanding, Fixed)` so a larger font or DPI makes the box **wider** instead of squeezing its content. Rule going forward: **no numeric spin box may be pinned narrower than `cfg.SPIN_MIN_W`.** **(2) Inconsistent height when the scope is set to "My favourites".** That hint line is one row longer than the others, which pushed the panel's `sizeHint` from 598 to 628 while the window stayed at a hard-coded `resize(660, 560)`; the layout was compressed, the `QGroupBox` was squeezed 30px below its `sizeHint`, and the spin boxes inside overflowed **on top of the "weight meaning" note below**. Fixed with a double safeguard: all three hint texts were padded to **two rows** and `lb_scope_tip` was pinned to a constant two-row height via `setMinimumHeight(fontMetrics().lineSpacing() * 2 + 2)`; and a new `AutoFillDialog._fit_height()` sizes the dialog from `sizeHint` (capped at 860) with `QTimer.singleShot(0, self._fit_height)` re-fitting at the end of `_refresh_est()`. Measured: switching between every scope now moves no block by even 1px. **(3) Overlapping number UI in the smart-recommendation "guide vector" row.** Same root cause as (1) — the guide row used `setFixedWidth(72)`, **59px short of the 131px minimum**, the worst of the three. Measured after the fix: `guide_w` goes from 72px to 131px and its spacing to the "Boost" button is normal again. The new probe asserts, with the real stylesheet loaded, that every spin box is at least its own `minimumSizeHint`, that no two sibling widgets intersect, that no child exceeds its container, and that switching scope moves no block by more than 2px. Before the fix it reported 10 crushed spin boxes plus a group-box/note intersection; after, zero issues. **While in there**: the spin boxes' up/down stepper arrows were nearly invisible on the dark theme (an empty grey slab), so `::up-button` / `::down-button` now carry colours plus hover/pressed states. Two QSS traps hit along the way and written down so nobody retries them — (a) `border-*: transparent` **does not** build a triangle in Qt (it has no browser-style "zero size + transparent borders" trick; you only get a solid square), and (b) `image: none` also removes Qt's built-in arrow glyph, leaving blank space. Conclusion: keep the native arrow and just recolour it with `color`.
- **v1.34.0** (2026-09-24) — Two new features, both in **smart recommendations**. **(1) "Auto-fill from portrait" is no longer hard-wired.** The old implementation was a fixed plan: library-wide scope, top 30 tags, top 10 of everything else, weights 1.5 / 1.3 / 1.2 — the user had no say. Clicking the button now opens an `AutoFillDialog` panel where you choose the **statistics scope** (all libraries / **my favourites** / a specific library, reusing `insight.Portrait`'s `library` + `favorites_only`), the **top-N per dimension** (1–200), the **weight per dimension** (0–5) and which dimensions to enable at all; the choice is **remembered** in `config.Settings.autofill` (with a whitespace/type-sanitising `_clean_autofill()` whitelist and a `set_autofill()` writer, both wired into `load()`/`save()`), and a **"Preview what would be written…"** button lets you inspect the entries without touching the database. **Defaults deliberately equal the old hard-coded plan**, so existing users see no behaviour change. **(2) A "guide vector" box next to "Shuffle".** Type an actor, tag, studio or series name and *this batch* leans towards it. It sits in the **same toolbar row as the "Shuffle" button, immediately to its left** (the left-hand info label is given `setMinimumWidth(0)` so it shrinks first rather than pushing the box out of view). The dimension is **auto-detected** (`recommend.resolve_guide_token()`: `people` table first, then tags/studios/series split out of `genres`; an unrecognised word prompts a dimension picker instead of being silently dropped), the weight is adjustable (0.5–10, default 2.0), several guides can be stacked, and each shows as a removable chip. The guide is injected by `_apply_guides()` **after** `build_profile`'s normalisation — before it, the `max=1` rescale would flatten the boost into nothing. It applies **to this batch only**: nothing is written to `vector_overrides` or to settings, so it is independent of the permanent weights edited in the vector editor. Measured on a real 49,668-title library (24 per batch, target さつき芽衣, baseline 1/24 = **4.2%**): weight 0.5 → 16.7%, 1.0 → 45.8%, 1.5 → 58.3%, **2.0 (default) → 66.7%**, 3.0 → 91.7%, 4.0 → 100%. Two real defects found by probing were also fixed: doing **substring** matching for people meant typing **巨乳** was captured by an actual performer named 「自己肯定感低めの色白巨乳立ちんぼ·N」 (people now match only exactly or by prefix), and unbounded substring matching meant typing **S** hit every S-initial term (prefixes now need ≥2 characters and substrings ≥3, with candidates ranked by match tier → closeness to the input → popularity, so「S1」correctly resolves to the studio `S1 NO.1 STYLE`).
