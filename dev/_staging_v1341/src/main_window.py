...
# v1.34.1 修复：QSpinBox / QDoubleSpinBox 的**最小可用宽度**见 `cfg.SPIN_MIN_W`。
# 此前引导行写 setFixedWidth(72)，比 Qt 实测 minimumSizeHint(131) 少 59px，
# 结果数字与箭头被挤在一格 → 数字贴边、右侧箭头被裁（用户截图红框处）。
...
class MainWindow(QMainWindow):
    ...
    def _guide_bar(self, row):
        ...
        # ⚠️ v1.34.1 修复：不要再写 setFixedWidth(72)。
        # QSS 的 `padding: 5px 10px` 已经把内容区收窄，`0.5~10.0` + 1 位小数 +
        # 上下箭头后，这台机器上实测 `minimumSizeHint().width()` 是 **131px**。
        # 原来给 72px 比最小需求少 59px → Qt 把数字与箭头挤在一起，
        # 数字贴边、右侧箭头区被裁（用户截图红框里 `[2.0▾]` 的样子）。
        # 正确做法：按 QSS 放行的宽度给足，并提升为**水平方向可伸展**，
        # 这样字体/DPI 变化时是变宽而不是压缩内容。
        self.guide_w.setMinimumWidth(cfg.SPIN_MIN_W)
        self.guide_w.setMaximumWidth(cfg.SPIN_MAX_W)
        self.guide_w.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
