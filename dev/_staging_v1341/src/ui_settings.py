...
class AutoFillDialog(GlassDialog):
    ...
    def _build(self):
        ...
        # v1.34.1 修复（用户反馈 2）：这段提示「我的收藏」是 2 行、其他范围是 1 行。
        # 行数一变，整个面板下方所有控件就上下跳 30px，看起来就是「高度存在问题」。
        # 这里把它**锁成恒定两行高**：所有范围的提示都补成两行，高度不再随选择变化。
        self.lb_scope_tip.setMinimumHeight(
            self.lb_scope_tip.fontMetrics().lineSpacing() * 2 + 2)
    ...
    def _refresh_est(self, *_):
        ...
        # v1.34.1：三档文案都写成 **两行**，配合 lb_scope_tip 的固定两行高，
        # 保证切换范围时下方控件位置完全不跳。
        if fav:
            self.lb_scope_tip.setText(
                "只统计已收藏的影片 —— 生成的权重要跟收藏口径一致时选这个。\n"
                "（收藏数少的话，画像会更聚焦你自己的口味。）")
        elif lib:
            self.lb_scope_tip.setText(
                f"只统计媒体库「{lib}」里的作品。\n"
                "（适合「这个库我口味不一样」，权重不会串到其他库。）")
        else:
            self.lb_scope_tip.setText(
                "统计索引里的全部作品（范围最广）。\n"
                "（适合「我就是想按整体口味找片」，不偏向任何一个库。）")
