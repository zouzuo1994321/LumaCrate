...
def mask_iou(a_img, a_box, b_img, b_box, thr=60):
    """两张图里各取一块矩形，二值化（亮于底色=前景）后算 IoU —— 形状比对。"""
    def _mask(img, box):
        x0, y0, x1, y1 = box
        w, h = x1 - x0, y1 - y0
        m = [[1 if lum(img.pixelColor(x, y)) >= thr else 0
              for x in range(x0, x1)] for y in range(y0, y1)]
        return m, w, h
    ma, wa, ha = _mask(a_img, a_box)
    mb, wb, hb = _mask(b_img, b_box)
...
def main():
                ...
                if img is not None:
                    ...
                    if len(prow) >= 8:
                        ...
                        segs = count_h_segments(img, BY0, BY1, BX0, BX1,
                                                thresh=52, gap=10, min_w=16)
                        segs = [s for s in segs if s >= 16]
    ...
    if about_pt:
        ...
        if not ab:
            ...
            if aimg is not None:
                # 联系图标条在对话框底部（「联系作者：」那一行）
                # 从底部往上找一条「4 个宽度相近的亮块」的行带
                bands = find_button_bands(aimg, 0, aw, int(ah * 0.60), ah,
                                          thresh=55, min_run=6)
                # 图标按钮本身边框暗（约 #3a3634），主要靠**图标前景**成带；
                # 放宽高度到 16~56，避免恰好卡在边界外。
                cands = [b for b in bands if 16 <= (b[1] - b[0]) <= 56]
                print("   关于窗底部候选按钮带 %d 条：%s" % (len(cands), cands[:6]),
                      flush=True)
                picked = None
                for b in sorted(cands, key=lambda t: -(t[3] - t[2])):
                    segs = count_h_segments(aimg, b[0], b[1] + 1, 0, aw,
                                            thresh=55, gap=8, min_w=14)
                    segs = [s for s in segs if s >= 14]
                    if len(segs) >= 4:
                        picked = (b, segs)
                        break
                if picked is None:
                    check("反馈 3 · 找到 4 枚联系图标所在行带", False,
                          "候选 %s" % cands[:4])
                else:
                    b, segs = picked
                    check("反馈 3 · 找到 4 枚联系图标所在行带", True,
                          "band=%s segs=%s" % (b, segs))
                    # 逐图标裁一块，量「非底色像素」+ 与参考剪影 IoU
                    refs = load_ref_logos()
                    kinds = ["github", "bilibili", "weibo", "mail"]
                    # 图标按钮是 34 宽 30 高、间距 10；用等距推算中心
                    ycen = (b[0] + b[1]) // 2
                    x_start = b[2]
                    # 从行带最左亮块起，按 44px 步长取 4 个中心
                    centers = []
                    x = x_start
                    for k in range(4):
                        centers.append((x + 17, ycen))
                        x += 44
                    for k, (cx, cy) in zip(kinds, centers):
                        box = crop_icon(aimg, cx, cy, r=15)
                        fg = 0
                        tot = 0
                        ...
                        if ref is not None:
                            ...
                            iou = mask_iou(aimg, box, ref, rb, thr=60)
                            print("       %-9s 与参考剪影 IoU=%.3f" % (k, iou), flush=True)
                            if k != "mail":
                                check("反馈 3 · %s 形状与平台 logo 一致（IoU>0.55）" % k,
                                      iou > 0.55, "%.3f" % iou)
                            else:
                                check("反馈 3 · 邮箱仍是自绘（与平台 logo 明显不同）",
                                      iou < 0.55, "%.3f" % iou)
